...
Ray::Ray(Vector3D<float> position, Vector2D<float> screen_res, RenderBuffer* rend_buff, std::vector<SceneNode*> node)
{
...
auto Ray::calculateReflexion() -> void
{
    /*this->reflexionCalc = -DOT(this->normal, this->direction)
        
    std::cout << "ReflexionCalc = " << this->reflexionCalc << std::endl;

    this->reflexionRay = this->direction + (this->normal * this->reflexionCalc * 2.f);
