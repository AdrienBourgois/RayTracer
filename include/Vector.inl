...
template <typename T>
auto Vector3D<T>::cast(Vector3D<float> vec) -> Vector3D
{
	Vector3D<uint8> vector;
	
	vector.x = static_cast<uint8> (vec.x);
	vector.y = static_cast<uint8> (vec.y);
	vector.z = static_cast<uint8> (vec.z);
